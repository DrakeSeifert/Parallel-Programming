...
main( int argc, char *argv[ ] )
{
 ...
 for(int a = 0; a < NUMTESTS; a++) {
   ...
   {
       //Choose coarse-grained parallelism and (static or dynamic) scheduling
       ...
       for( int i = 0; i < NUMBODIES; i++ )
       {
            ...
            //Choose fine-grained parallelism and (static or dynamic) scheduling
   ...
   double mbcps = ((double)NUMBODIES*NUMBODIES*NUMSTEPS) / (time1 - time0) / 1000000.;
   sum += mbcps;
   if(mbcps > max)
      max = mbcps;
   ...
   double avg = sum / (double)NUMTESTS;
   printf("Peak Performance = %8.2lf MegaBodies Compared / Sec\n", max);
   printf(" Avg Performance = %8.2lf MegaBodies Compared / Sec\n", avg);
